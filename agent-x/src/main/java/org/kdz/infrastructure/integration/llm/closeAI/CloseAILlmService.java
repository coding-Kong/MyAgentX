package org.kdz.infrastructure.integration.llm.closeAI;
...
import org.kdz.domain.llm.model.LlmMessage;
import org.kdz.domain.llm.model.LlmRequest;
import org.kdz.domain.llm.model.LlmResponse;
import org.kdz.infrastructure.integration.llm.AbstractLlmService;
...
@Service
public class CloseAILlmService extends AbstractLlmService {

    private final Logger logger = LoggerFactory.getLogger(CloseAILlmService.class);


    public CloseAILlmService(
            @Value("${llm.provider.providers.closeAI.name:CloseAI}") String providerName,
            @Value("${llm.provider.providers.closeAI.api-url:https://api.openai-proxy.org/v1/chat/completions}") String apiUrl,
            @Value("${llm.provider.providers.closeAI.api-key:}") String apiKey,
            @Value("${llm.provider.providers.closeAI.model:gpt-4o-mini}") String defaultModel,
            @Value("${llm.provider.providers.closeAI.timeout:30000}") int timeout) {
        super(providerName, apiUrl, apiKey, defaultModel, timeout);

        if (apiKey == null || apiKey.isEmpty()) {
            logger.warn("CloseAI API密钥未配置，请通过环境变量CloseAI_API_KEY设置");
        } else {
            logger.info("初始化CloseAI服务，默认模型: {}", defaultModel);
        }
    }
    ...
    public LlmResponse chat(LlmRequest request) {
        ...
        try {
            logger.info("发送请求到CloseAI服务, 模型: {}, 消息数: {}",
            ...
            logger.debug("CloseAI服务响应长度: {}", responseBody.length());
            return parseResponse(responseBody);
        } catch (Exception e) {
            logger.error("调用CloseAI服务出错", e);
    ...
    public void streamChat(LlmRequest request, StreamResponseHandler handler) {
        ...
        try {
            logger.info("发送流式请求到CloseAI服务, 模型: {}, 消息数: {}",
                    request.getModel(), request.getMessages().size());
            ...
            logger.error("调用CloseAI流式服务出错", e);
    ...
    public List<String> chatStreamList(LlmRequest request) {
        ...
        try {
            ...
            logger.info("CloseAI流式响应完成，共返回 {} 个块", chunks.size());
            return chunks;
