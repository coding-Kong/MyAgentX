package org.kdz.application.conversation.service;
...
import org.kdz.application.conversation.dto.StreamChatResponse;
...
public class ConversationService {
    ...
    public void kchatStream(StreamChatRequest request, BiConsumer<StreamChatResponse, Boolean> responseHandler) {
        ...
        try {
            // 检查LLM服务是否为closeAILlmService以使用其回调接口
            if (llmService instanceof CloseAILlmService) {
                logger.info("使用CloseAI的真实流式响应");
                CloseAILlmService closeAILlmService = (CloseAILlmService) llmService;

                // 使用回调接口
                closeAILlmService.streamChat(llmRequest, (chunk, isLast) -> {
                    StreamChatResponse response = new StreamChatResponse();
